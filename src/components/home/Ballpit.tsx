...
const iconUrls = [
  ...
  'https://cdn.simpleicons.org/tesla/white',
  'https://cdn.simpleicons.org/nvidia/white',
  'https://cdn.simpleicons.org/flutter/white',
  'https://cdn.simpleicons.org/mongodb/white',
  'https://cdn.simpleicons.org/firebase/white',
  'https://cdn.simpleicons.org/docker/white',
  'https://cdn.simpleicons.org/kubernetes/white',
...
class x {
  ...
  #p() {
    ...
    try {
      ...
      console.error('Three: Failed to create WebGLRenderer', err);
      throw err;
    }
  }
  #g() {
  ...
  constructor(e) {
    ...
    this.onBeforeCompile = e => {
      ...
      `.replace(
        ...
        `
        #include <uv_vertex>
        vIconIndex = aIconIndex;
        vLocalPos = position; // Local position of the vertex
        `
      );
      ...
      e.fragmentShader = e.fragmentShader.replace(
        ...
        #include <map_fragment>
        if (vIconIndex >= 0.0) {
          // Use planar mapping from the front (z-axis) to avoid sphere UV stretching
          vec2 iconUv = vLocalPos.xy; 
          
          // Scale and center the icon on the ball (Lower scale = Larger icon)
          float iconScale = 0.6; 
          iconUv = iconUv * iconScale + 0.5;
          
          if (iconUv.x >= 0.0 && iconUv.x <= 1.0 && iconUv.y >= 0.0 && iconUv.y <= 1.0 && vLocalPos.z > 0.0) {
            float cols = uAtlasSize.x;
            float rows = uAtlasSize.y;
            float col = mod(vIconIndex, cols);
            float row = floor(vIconIndex / cols);
            vec2 atlasUv = vec2(
              (col + iconUv.x) / cols,
              (row + (1.0 - iconUv.y)) / rows
            );
            vec4 iconColor = texture2D(uAtlas, atlasUv);
            
            float iconAlpha = iconColor.a * 0.9;
            diffuseColor.rgb = mix(diffuseColor.rgb, vec3(1.0), iconAlpha);
            totalEmissiveRadiance += vec3(iconAlpha * 0.6); 
          }
        }
        `
      );
...
async function createIconAtlas() {
  ...
  const size = 256;
  const cols = 4;
  const rows = Math.ceil(iconUrls.length / cols);
  canvas.width = cols * size;
  canvas.height = rows * size;
  const ctx = canvas.getContext('2d');

  const promises = iconUrls.map((url, i) => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        // Draw with minimal padding to maximize icon size
        const pad = size * 0.02;
...
class Z extends d {
  constructor(e, t = {}) {
    ...
    const iconIndices = new Float32Array(i.count);
    for (let j = 0; j < i.count; j++) {
      // Assign an icon index to every ball for maximum visibility
      iconIndices[j] = Math.floor(Math.random() * iconUrls.length);
    }
    o.setAttribute('aIconIndex', new InstancedBufferAttribute(iconIndices, 1));
    ...
    createIconAtlas().then(({ texture, cols, rows }) => {
      r.uniforms.uAtlas.value = texture;
      r.uniforms.uAtlasSize.value.set(cols, rows);
    });
  }
  #S() {
